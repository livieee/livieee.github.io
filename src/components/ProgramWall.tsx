...
export function ProgramWall() {
  ...
  return (
    <Link
      ...
      className="wall-board group/wall relative block aspect-[21/9] w-full cursor-none overflow-hidden rounded-[1.4rem] bg-gradient-to-br from-lavender/45 via-cream-soft to-blush/30"
      onPointerMove={(e) => {
    ...
    >
      ...
      {/* 便签 */}
      <span
        className="absolute bottom-[3%] left-[2%] w-[27%] -rotate-2 rounded-md border border-dashed border-rose/50 bg-white/95 px-2 py-1.5 shadow-sm"
        style={{ animation: 'annot-in .6s .54s ease-out both' }}
      >
        <span className="block font-hand text-[14px] leading-tight text-plum">
          make the room work ✦
        </span>
      </span>
